    ...
    const checkConditions = setInterval(() => {
        ...
        if (targetUrlPattern.test(currentUrl)) {
            ...
            setTimeout(() => {
                ...
                // Refresh the parent tab if it's a /c/ URL
                chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
                    const parentTabUrl = tabs[0].url;
                    if (parentTabUrl.includes("/c/")) {
                        setTimeout(() => {
                            chrome.tabs.reload(tabs[0].id);
                            console.log("Refreshed the parent tab with /c/ in the URL.");
...
// Monitor for the parent tab and ensure it doesn't close if it has /c/ in the URL
function monitorParentTab() {
    if (window.location.href.includes("/c/")) {
        setTimeout(() => {
            const catshadowElements = document.querySelectorAll('.catshadow.adshelper');
            if (catshadowElements.length === 0) {
                setTimeout(() => {
                    console.log("No catshadow.adshelper elements found. Refreshing the page.");
                    location.reload();  // Refresh the page
                }, 4000);  // Wait for 4 seconds before refreshing
            }
        }, 4000);  // Initial 4-second wait after the page load
    }
}

// Execute the parent tab monitor
monitorParentTab();
